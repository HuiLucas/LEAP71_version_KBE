...
namespace Leap71
{
    ...
    namespace LatticeLibrary
    {
	    ...
	    public class CuboidCell : IUnitCell
        {
            ...
            {
                // wireframe
                Sh.PreviewLine(new List<Vector3>() { m_aCornerPoints[0], m_aCornerPoints[1], m_aCornerPoints[2], m_aCornerPoints[3], m_aCornerPoints[0] }, Cp.strBlack);
                Sh.PreviewLine(new List<Vector3>() { m_aCornerPoints[4], m_aCornerPoints[5], m_aCornerPoints[6], m_aCornerPoints[7], m_aCornerPoints[4] }, Cp.strBlack);
                Sh.PreviewLine(new List<Vector3>() { m_aCornerPoints[0], m_aCornerPoints[4] }, Cp.strBlack);
                Sh.PreviewLine(new List<Vector3>() { m_aCornerPoints[1], m_aCornerPoints[5] }, Cp.strBlack);
                Sh.PreviewLine(new List<Vector3>() { m_aCornerPoints[2], m_aCornerPoints[6] }, Cp.strBlack);
                Sh.PreviewLine(new List<Vector3>() { m_aCornerPoints[3], m_aCornerPoints[7] }, Cp.strBlack);

                //// corners
                // Sh.PreviewPointCloud(aGetCornerPoints(), 0.1f, Cp.strRed);

                //// faces
                // string strColor = Cp.strRandom();
